# Claude Code Project Instructions — serde-json-kotlin

## Project Overview

This is **serde-json-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`serde_json`](https://crates.io/crates/serde_json).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/serde_json/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/serde_json>
